...
void saveFrame(void)
{
	...
	KeyFrame[FrameIndex].FRLegs = FRLegs;
	KeyFrame[FrameIndex].FLLegs = FLLegs;
...
void resetElements(void)
{
	...
	FRLegs = KeyFrame[0].FRLegs;
	FLLegs = KeyFrame[0].FLLegs;
...
void interpolation(void)
{
	...
	KeyFrame[playIndex].FRLegsInc = (KeyFrame[playIndex + 1].FRLegs - KeyFrame[playIndex].FRLegs) / i_max_steps;
	KeyFrame[playIndex].FLLegsInc = (KeyFrame[playIndex + 1].FLLegs - KeyFrame[playIndex].FLLegs) / i_max_steps;
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Previo 12 - Angel Flores", nullptr, nullptr);
	...
	for (int i = 0; i < MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].FRLegs = 0;
		KeyFrame[i].FRLegsInc = 0;
		KeyFrame[i].FLLegs = 0;
		KeyFrame[i].FLLegsInc = 0;
		KeyFrame[i].RLegs = 0;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		model = glm::rotate(model, glm::radians(FLLegs), glm::vec3(1.0f, 0.0f, 0.0f)); 
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		...
		model = glm::rotate(model, glm::radians(FRLegs), glm::vec3(1.0f, 0.0f, 0.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
...
void Animation() {
	...
	if (play)
	{
		...
		else
		{
			...
			FRLegs += KeyFrame[playIndex].FRLegsInc;
			FLLegs += KeyFrame[playIndex].FLLegsInc;
