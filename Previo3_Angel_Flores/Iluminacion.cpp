...
// Light attributes
glm::vec3 lightPos(0.5f, -1.0f, -1.50f);
glm::vec3 lightPos2(3.0f, -1.0f, -3.0f);
...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Previo 8 Angel Flores", nullptr, nullptr);
    ...
    // Load models
    Model red_dog((char*)"Models/Campamento/perro.obj");
    ...
    image = stbi_load("Models/textures/Texture_1.png", &textureWidth, &textureHeight, &nrChannels, 0);
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		//Primer renderizado con la primera luz
        lightingShader.Use();
		...
		// Draw the loaded model with the first light
        glm::mat4 model(1);
		...
		//Segundo renderizado con la segunda luz
		//Habilitar blending para sumar la iluminacion
        glEnable(GL_BLEND);
        glBlendFunc(GL_ONE, GL_ONE);
        glDepthFunc(GL_EQUAL);  // Solo dibujar donde ya se dibujó

        lightingShader.Use();
        glUniform3f(glGetUniformLocation(lightingShader.Program, "light.position"),
            lightPos2.x + movelightPos2, lightPos2.y + movelightPos2, lightPos2.z + movelightPos2);
        glUniform3f(glGetUniformLocation(lightingShader.Program, "viewPos"),
            camera.GetPosition().x, camera.GetPosition().y, camera.GetPosition().z);

        // Propiedades de la segunda luz (sin componente ambient para evitar duplicación)
        ...
        // Draw the loaded model with second light
        model = glm::mat4(1);
        model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
        glUniformMatrix4fv(glGetUniformLocation(lightingShader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
        glBindVertexArray(VAO);
        ...
        // Deshabilitar blending y restaurar depth function
        glDisable(GL_BLEND);
        glDepthFunc(GL_LESS);
